...
class eomSSTN(Utilities.EOMS):
    ...
    def __new__(cls, node):
        ...
        instance.CoP = Utilities.Vector(node.Attributes["cop"].Value)
        ...
        instance.Ivec = Utilities.Vector(node.Attributes["moi"].Value) # Can Vectors be initialized from the XML file? Need to figure out -AJ
        ...
        instance.Imat = Utilities.Matrix[System.Double](3,3)
        instance.Imat[1,1] = instance.Ixx
        instance.Imat[2,2] = instance.Iyy
        instance.Imat[3,3] = instance.Izz
        instance.CoM = Utilities.Vector(node.Attributes["com"].Value)
        instance.WHEELTORQUE_KEY = Utilities.StateVarKey[Utilities.Matrix[System.Double]](instance.AssetName + '.' + 'rxwheel_torque')
        instance.MAGTORQDIPOLE_KEY = Utilities.StateVarKey[Utilities.Matrix[System.Double]](instance.AssetName + '.' + 'magtorq_dipole')
        instance.IsWheelsVec = Utilities.Vector(node.Attributes["iswheels"].Value)
        instance.IsWheels = Utilities.Matrix[System.Double](3,3)
    ...
    def PythonAccessor(self, t, y, param, environment):
        ...
        wwyb = y[15]
        wwzb = y[16]
        Reci = Utilities.Matrix[System.Double](3,1)
        Reci[1] = xeci
        Reci[2] = yeci
        Reci[3] = zeci
        Veci = Utilities.Matrix[System.Double](3,1)
        Veci[1] = vxeci
        Veci[2] = vyeci
        Veci[3] = vzeci
        epsbeci = Utilities.Matrix[System.Double](3,1)
        epsbeci[1] = qb1
        epsbeci[2] = qb2
        epsbeci[3] = qb3
        qbeci = Utilities.Quat[System.Double](qb0,epsbeci)
        wbeci = Utilities.Matrix[System.Double](3,1)
        wbeci[1] = wxbeci
        wbeci[2] = wybeci
        wbeci[3] = wzbeci
        wwb = Utilities.Matrix[System.Double](3,1)
        wwb[1] = wwxb
        wwb[2] = wwyb
        wwb[3] = wwzb
        epscross = Utilities.Matrix[System.Double].CrossMatrix(epsbeci)
        etaI = Utilities.Matrix[System.Double](3,3)
        etaI[1,1] = qb0
        etaI[2,2] = qb0
        etaI[3,3] = qb0
        ...
        dy = Utilities.Matrix[System.Double](16,1)
        dy[1] = vxeci
        dy[2] = vyeci
        dy[3] = vzeci
        ...
        dy[5] = acceleration[2]
        dy[6] = acceleration[3]
        dy[7] = -0.5*Utilities.Matrix[System.Double].Dot(epsbeci,wbeci)
        dy[8] = epsbecidot[1]
        dy[9] = epsbecidot[2]
        dy[10] = epsbecidot[3]
        T_dist = self.CalcMoments(Reci,Veci,qbeci,T_control,M_dipole)
        omegaDot = Utilities.Matrix[System.Double].Inverse(self.Imat)*(T_dist-Utilities.Matrix[System.Double].Cross(wbeci,self.Imat*wbeci)) # Correct formula for body rate integration? Or need wheel momentum? -AJ
        dy[11] = omegaDot[1]
        dy[12] = omegaDot[2]
        dy[13] = omegaDot[3]
        omegaWDot = Utilities.Matrix[System.Double].Inverse(self.IsWheels)*(-T_control-Utilities.Matrix[System.Double].Cross(wbeci,self.IsWheels*wwb)) # Fairly confident, ask Mehiel anyways - AJ
        dy[14] = omegaWDot[1]
        dy[15] = omegaWDot[2]
        dy[16] = omegaWDot[3]
        return dy
    ...
    def CalcGravityForce(self,r):
        mu = 398600.4418
        r3 = Utilities.Matrix[System.Double].Norm(r)^3
        agrav = Utilities.Matrix[System.Double](3,1)
        agrav[1] = -mu*r[1]/r3
        agrav[2] = -mu*r[2]/r3
        agrav[3] = -mu*r[3]/r3
        return agrav
    ...
    def CalcJ2Force(self,r):
        ...
        J2 = 1.0826269e-3
        rE = 6378.137
        rnorm = Utilities.Matrix[System.Double].Norm(r)
        aJ2 = Utilities.Matrix[System.Double](3,1)
    ...
    def CalcDragForce(self,r_eci,v_eci):
        ...
        vnorm = Utilities.Matrix[System.Double].Norm(v_eci)
    ...
    def CalcDragMoment(self,r_eci,v_eci):
        ...
        T_d = Utilities.Matrix[System.Double].Cross(self.CoP,fdrag)
        return T_d

    def CalcMagMoment(self,r_eci,JD,M_dipole,qb_eci):
        bField = self.CalcCurrentMagField(r_eci,JD)
        bFieldBody = Utilities.Quat.Rotate(qb_eci,bField)
        magMoment = Utilities.Matrix[System.Double].Cross(M_dipole,bFieldBody)
        return magMoment
    ...
    def CalcGravGradMoment(self,r_eci,qb_eci):
        ...
        rb = Utilities.Quat.Rotate(qb_eci,r_eci)
        T_g = 3.0*mu*Utilities.Matrix[System.Double].Cross(rb,self.Imat*rb)/rnorm
        return T_g

    def CalcAtmosDens(self,r_eci):
        rE = 6378.137
        h = Utilities.Matrix[System.Double].Norm(r_eci) - rE
        # Based on exp. atmosphere model from Vallado Table 8-4
        atmMatrix = Utilities.Matrix[System.Double](28,3)
    ...
    def CalcRotMat(self, q):
        eta = q._eta
        eps = q._eps
        idmat = Utilities.Matrix[System.Double].Eye(3)
        epstrans = Utilities.Matrix[System.Double].Transpose(eps)
        Q = (2*eta^2 - 1)*idmat + 2*eps*epstrans - 2*eta*epscross
        return Q

    def CalcInertiaMatrix(self):
        InerMat = Utilities.Matrix[System.Double](3,3)
    ...
    def CalcWheelInertiaMatrix(self):
        IswMat = Utilities.Matrix[System.Double](3,3)
